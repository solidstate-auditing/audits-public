...
| -------- | -------- | -------- |
| **Access Controls**  | **Sufficient** | Access control mechanisms and re-entrancy checks are properly implemented. |
| **Architecture and Design**  | **Weak** | The code unnecessarily overuses structures such as `BTreeMap` and `HashMap` for storing single values. This increases complexity and overhead, making the code harder to understand and maintain. |
...
| **Upgradeability**  | **Weak** | Since the canister will be blackholed, its current state must be robust. The issues outlined in the executive summary must be addressed before proceeding, as there will be no way to apply fixes after deployment. |
| **Documentation**  | **Weak** | While the whitepaper provides a high-level overview of the protocol, the code lacks inline documentation, making it difficult to understand and maintain. |
| **Testing and Verification**  | **Weak** | There are no unit tests or integration tests. The team relies entirely on manual testing, which is inadequate for a system that cannot be modified after blackholing. |
...
### **SS-ALEX-02 Unused Computed Default Subaccount Enables Arbitrary Token Minting**
...
### **SS-ALEX-03 Potential Incomplete Reward Distribution Due to Insufficient Cycle Checks**
...
### **SS-ALEX-04 External Call Failure in `within_max_limit` Enables Burn Operation Denial**
...
### **SS-ALEX-05 Inadequate Logging via `print` and `println` Instead of Auditable Journals**
...
### **SS-ALEX-06 Redundant `is_canister` Guard on `update` Functions**
...
| --- | --- | --- | --- | --- |
| SS-ALEX-01| Very High | Fixed | N/A | The team has acknowledged the issue and will update the whitepaper accordingly. |
| SS-ALEX-02| Very High | Fixed | 47ff4d8f0f5553fb5e929e3e4111ec1ad842799b | The team decided to use the default subaccount for all users. |
| SS-ALEX-03| Medium | Not Fixed | N/A | The team has stated that they are confident they will never run out of cycles and will keep the canister functional. |
| SS-ALEX-04| Low | Fixed | 96a893c08f5384372465a943c921833ad8504a31 | The return amount is adjusted. |
| SS-ALEX-05| Low | Not Fixed | N/A | N/A|
| SS-ALEX-06| Low | Fixed | 96a893c08f5384372465a943c921833ad8504a31 | The `is_canister` check is removed and the functions are only available internally. |
